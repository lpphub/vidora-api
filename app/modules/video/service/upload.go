...
import (
	...
	model2 "vidora-api/app/modules/video/model"
...
func (s *UploadService) Init(ctx context.Context, req *model2.UploadInitReq) (*model2.UploadInitResp, error) {
	uploadFile, err := s.repo.FindUploadFileByMD5(ctx, req.MD5)
	if err == nil {
		return &model2.UploadInitResp{
			Exists:  true,
	...
	newSession := &model2.UploadSession{
		UploadID:       uploadID,
		MD5:            req.MD5,
...
func (s *UploadService) UploadChunk(ctx context.Context, req *model2.UploadChunkReq) (*model2.UploadChunkResp, error) {
	...
	return &model2.UploadChunkResp{
		Uploaded:    true,
		ChunkNumber: req.ChunkNumber,
	}, nil
}

func (s *UploadService) Merge(ctx context.Context, req *model2.UploadMergeReq) (*model2.UploadMergeResp, error) {
	session, err := s.repo.GetSession(ctx, req.UploadID)
	if err != nil {
		return nil, errs.ErrUploadSessionNotFound
	}
	...
	return &model2.UploadMergeResp{
		FileKey: fileKey,
		URL:     storageURL,
	}, nil
}

func (s *UploadService) Status(ctx context.Context, uploadID string) (*model2.UploadStatusResp, error) {
	...
	return &model2.UploadStatusResp{
...
func (s *UploadService) calculateETA(session *model2.UploadSession) int {
